...
Functions:
    ...
    - get_severe_weather_alerts(location, days): Check for severe weather conditions
...
def get_forecast(location: str, days: int = 7) -> Dict:
    ...
    """
    # Convert and validate days parameter (handle string/float from MCP calls)
    try:
        days = int(float(days))  # Convert string or float to int
...
def get_severe_weather_alerts(location: str, days: int = 7) -> Dict:
    ...
    Analyzes forecast data to identify potential severe weather:
    - Extreme temperatures (< -10°C or > 38°C)
    - Heavy precipitation (> 80% probability)
    - Severe weather codes (thunderstorms, heavy rain/snow, hail)
    
    Args:
        location: Location name (city, address, etc.)
        days: Number of days to check (1-16, default 7)
    ...
    """
    # Severe weather codes (WMO codes for dangerous conditions)
    SEVERE_CODES = {
        65: ("HIGH", "Heavy Rain"),
        71: ("MODERATE", "Snow"),
        73: ("MODERATE", "Moderate Snow"),
        75: ("HIGH", "Heavy Snow"),
        77: ("MODERATE", "Snow Grains"),
        82: ("HIGH", "Violent Rain Showers"),
        85: ("MODERATE", "Snow Showers"),
        86: ("HIGH", "Heavy Snow Showers"),
        95: ("HIGH", "Thunderstorm"),
        96: ("HIGH", "Thunderstorm with Hail"),
        99: ("HIGH", "Thunderstorm with Heavy Hail"),
    }
    ...
    # Check each day for severe conditions
    for day_forecast in forecast["daily_forecast"]:
        day_alerts = []
        
        # Check for extreme cold
        if day_forecast["temperature_max_celsius"] < -10:
            day_alerts.append({
                "severity": "HIGH",
                "type": "Extreme Cold",
                "description": f"Dangerously low temperatures expected: {day_forecast['temperature_max_celsius']}°C ({day_forecast['temperature_max_fahrenheit']}°F)"
            })
        
        # Check for extreme heat
        if day_forecast["temperature_max_celsius"] > 38:
            day_alerts.append({
                "severity": "HIGH",
                "type": "Extreme Heat",
                "description": f"Dangerously high temperatures expected: {day_forecast['temperature_max_celsius']}°C ({day_forecast['temperature_max_fahrenheit']}°F)"
            })
        
        # Check for heavy precipitation
        if day_forecast["precipitation_probability_percent"] > 80:
            day_alerts.append({
                "severity": "MODERATE",
                "type": "Heavy Precipitation",
                "description": f"Very high chance of precipitation: {day_forecast['precipitation_probability_percent']}%"
            })
        
        # Check for severe weather codes
        weather_code = day_forecast["weather_code"]
        if weather_code in SEVERE_CODES:
            severity, weather_type = SEVERE_CODES[weather_code]
            day_alerts.append({
                "severity": severity,
                "type": weather_type,
                "description": f"Severe weather expected: {day_forecast['weather_description']}"
            })
        
        # If any alerts for this day, add to main alerts list
        if day_alerts:
            # Combine all alerts for this day
            combined_description = "; ".join([alert["description"] for alert in day_alerts])
            highest_severity = "HIGH" if any(a["severity"] == "HIGH" for a in day_alerts) else "MODERATE"
            alert_types = ", ".join(set([alert["type"] for alert in day_alerts]))
            ...
            alerts.append({
                ...
                "temperature_max_celsius": day_forecast["temperature_max_celsius"],
                "temperature_max_fahrenheit": day_forecast["temperature_max_fahrenheit"],
                "precipitation_probability_percent": day_forecast["precipitation_probability_percent"],
                "weather_description": day_forecast["weather_description"]
            })
